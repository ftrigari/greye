...
func main() {
	...
	helloAppHandlers := factory.BuildAppHandlers()
	helloClHandlers := factory.BuildClusterHandlers()

	//msg := fmt.Sprintf("ClusterMonitor is working on %s server", networkInfo.LocalIp)
	//fmt.Println(msg)
	srv := server.NewServer(helloAppHandlers, helloClHandlers, configurator)
